use srvcs_hexagonalnumber::{api::Deps, config::Config, health, router, telemetry};
...
async fn main() {
    ...
    tracing::info!(addr = %cfg.bind_addr, env = %cfg.environment, "srvcs-hexagonalnumber listening");
    let deps = Deps {
